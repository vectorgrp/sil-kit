...
protected:
    Test_FlexrayController()
        ...
        , controllerConfigured(&participant, GetDummyConfigWithValues(), participant.GetTimeProvider())
    {
        controller.SetServiceDescriptor(controllerAddress);
        controller.SetDetailedBehavior(busSimAddress);
        controllerConfigured.SetServiceDescriptor(controllerConfiguredAddress);
        controllerConfigured.SetDetailedBehavior(busSimAddress);
...
protected:
    ...
    const ServiceDescriptor controllerConfiguredAddress{"p2", "n1", "c1", 5};
    const ServiceDescriptor busSimAddress{netsimName, "n1", "c1", 5};
...
TEST_F(Test_FlexrayController, send_controller_config)
{
    ...
    EXPECT_CALL(participant, SendMsg(&controller, netsimName, controllerCfg)).Times(1);
...
TEST_F(Test_FlexrayController, send_controller_config_override_identical)
{
    ...
    EXPECT_CALL(participant, SendMsg(&controllerConfigured, netsimName, configuredControllerCfg)).Times(1);
...
TEST_F(Test_FlexrayController, send_txbuffer_configupdate)
{
    ...
    EXPECT_CALL(participant, SendMsg(&controller, netsimName, expectedUpdate)).Times(1);
...
TEST_F(Test_FlexrayController, send_txbuffer_update)
{
    ...
    EXPECT_CALL(participant, SendMsg(&controller, netsimName, update)).Times(1);
...
{
    EXPECT_CALL(participant, SendMsg(&controller, netsimName, FlexrayHostCommand{FlexrayChiCommand::RUN})).Times(1);
...
{
    EXPECT_CALL(participant, SendMsg(&controller, netsimName, FlexrayHostCommand{FlexrayChiCommand::DEFERRED_HALT}))
        .Times(1);
...
{
    EXPECT_CALL(participant, SendMsg(&controller, netsimName, FlexrayHostCommand{FlexrayChiCommand::FREEZE})).Times(1);
...
{
    EXPECT_CALL(participant, SendMsg(&controller, netsimName, FlexrayHostCommand{FlexrayChiCommand::ALLOW_COLDSTART}))
        .Times(1);
...
TEST_F(Test_FlexrayController, send_all_slots_command)
{
    ...
    EXPECT_CALL(participant, SendMsg(&controller, netsimName, cmd)).Times(1);
...
{
    EXPECT_CALL(participant, SendMsg(&controller, netsimName, FlexrayHostCommand{FlexrayChiCommand::WAKEUP})).Times(1);
...
TEST_F(Test_FlexrayController, call_message_handler)
{
    ...
    EXPECT_CALL(callbacks, MessageHandler(&controller, ToFlexrayFrameEvent(message))).Times(1);
...
TEST_F(Test_FlexrayController, call_message_ack_handler)
{
    ...
    EXPECT_CALL(callbacks, MessageAckHandler(&controller, ToFlexrayFrameTransmitEvent(ack))).Times(1);
...
TEST_F(Test_FlexrayController, call_wakeup_handler)
{
    ...
    EXPECT_CALL(callbacks, WakeupHandler(&controller, wus)).Times(1);
    ...
    EXPECT_CALL(callbacks, WakeupHandler(&controller, wudop)).Times(1);
    ...
    EXPECT_CALL(callbacks, WakeupHandler(&controller, casMts)).Times(0);
...
TEST_F(Test_FlexrayController, call_pocstatus_handler)
{
    ...
    EXPECT_CALL(callbacks, PocStatusHandler(&controller, poc)).Times(1);
...
TEST_F(Test_FlexrayController, call_symbol_handler)
{
    ...
    EXPECT_CALL(callbacks, SymbolHandler(&controller, wus)).Times(1);

    FlexraySymbolEvent wudop;
    wudop.pattern = FlexraySymbolPattern::Wudop;
    EXPECT_CALL(callbacks, SymbolHandler(&controller, wudop)).Times(1);

    FlexraySymbolEvent casMts;
    casMts.pattern = FlexraySymbolPattern::CasMts;
    EXPECT_CALL(callbacks, SymbolHandler(&controller, casMts)).Times(1);
...
TEST_F(Test_FlexrayController, call_symbol_ack_handler)
{
    ...
    EXPECT_CALL(callbacks, SymbolAckHandler(&controller, ack)).Times(1);
...
TEST_F(Test_FlexrayController, call_cyclestart_handler)
{
    ...
    EXPECT_CALL(callbacks, CycleStartHandler(&controller, cycleStart)).Times(1);
