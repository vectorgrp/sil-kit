...
protected:
    ThreeEthControllerITest()
    {
        ...
        auto&& participant = setup.AddParticipant("EthWriter");
        auto&& eth = participant->AddEthernet("ETH1").WithLink("LINK1");

        //XXX testing trace sinks
        participant->AddTraceSink("EthWriterSink")
            .WithOutputPath("EthWriter.pcap")
            .WithType(ib::cfg::TraceSink::Type::PcapFile);
        eth.WithTraceSink("EthWriterSink");
