...
PCAP is supported natively in the IntegrationBus and the MDF4 file format is supported by the :ref:`VIBE-MDF4Tracing<mdf4tracing>` extension.
...
These data types map to specific bus frame types that are supported by MDF
standard channels, in contrast to the internal IB simulation control, synchronization and service specific auxiliary data types which will not be traced.
...
Then, the demo needs to run for some time to accumulate data frames and we need to shutdown it cleanly, which will result in a valid trace file.
...
In particular, some of the EthernetController's data transmission APIs are disabled to allow replaying data on unmodified participants.
...
IntegrationBus messages via the
``EthController::ReceiveIbMessage(ib::mw::EndpointAddress, const EthMessage&)``
callbacks, and will inject EthMessages extracted from a trace file instead.
...
should now always log a transmit ID of zero, because it's API calls are
disabled.
...
.. admonition:: Note
   ...
   When replaying is active on a service controller, invoking a data transmission API on the controller is disabled.
...
Other IntegrationBus API calls are not prevented, as the trace message replay relies on the colaboration of
...
.. list-table:: Trace and Replaying Data Types Supported
   ...
   * - LIN
     - ib::sim::lin::Frame
     - |MDF4|
     - Replay is only possible on a Master controller.
   * - Ethernet
     - ib::sim::eth::EthFrame
     - |MDF4|
       |PCAP|
     - PCAP does not support filtering the replay direction (receive/send).
       That is, the complete PCAP trace file is always replayed on the given
       controller.
   * - FlexRay
     - ib::sim::fr::FrMessage
     - |MDF4|
     - Requires the VIBE-NetworkSimulator.
   * - IO Ports
     ...
     - |MDF4|
     - Uses CANoe compatible encoding.
   * - GenericMessage
...
- Generic data types, such as Generic Messages and I/O ports are encoded in a
  CANoe compatible way.
  They are encoded as SystemVariables and a MDF4 trace file is acompanied by a
  *.vsysvar* definition file.
  This definition file has to be imported into the CANoe system variable
...
without the need to modify and recompile the particiapnt.
Please note, that only the listed methods for the types specified in :ref:`the
previous table<table-overview>` are affected in their runtime behavior.

The controller implementations use the following replay policies.
The ReceiveIbMessage code path usually triggers previously registered user
handlers.
These handlers are still invoked, but only with data replayed on the given
controller in *Receive* direction.

.. list-table:: API behavior during Replay
   :widths: 10 90
   :header-rows: 1
   
   * - Method
     - User calls disabled for Direction
   * - ``CanController::SendMessage``
     - Send, Both
   * - ``CanController::ReceiveIbMessage``
     - Receive, Both
   
   * - ``LinController::SendFrame``
     - Send, Receive, Both
   * - ``LinController::SendFrameHeader``
     - Send, Receive, Both
   * - ``LinController::SetFrameResponse``
     - Send, Receive, Both

   * - ``EthernetController::SendMessage``
     - Send, Both
   * - ``EthernetController::SendFrame``
     - Send, Both
   * - ``EthernetController::ReceiveIbMessage``
     - Receive, Both

   * - ``FrController::ReconfigureTxBuffer``
     - Send, Both
   * - ``FrController::UpdateTxBuffer``
     - Send, Both
   
   * - ``OutPort<MsgT>::Write``
     - Send, Both
   * - ``InPort<MsgT>::ReceiveIbMessage``
     - Receive, Both

   * - ``GenericPublisher::Publish``
     - Send, Both
   * - ``GenericSubscriber::ReceiveIbMessage``
     - Receive, Both
...
It gives access to the current simulation time, or as a fallback the wallclock time if no participant controller is configured.
...
The controllers have specific trace points where messages are pushed in to the
sinks.
...
thernet frames without meta data like the participand ID and controller ID from
...
information like the original VIB configuration, the transmission direction of a frame and information idenfitying the acquisition source within the VIB simulation setup.
...
Based on the configuration it creates replay controllers, configures them and
attaches them to a trace file with a matching logical data stream, known as a replay channel.
