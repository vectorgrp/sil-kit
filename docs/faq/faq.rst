...
It does not matter to SIL Kit, whether all simulation participants are running on the same kind of operating system.
...
.. admonition:: Note
   ...
   This example uses a different port, as a tool may have already opened a sil-kit-registry on the default port 8500.

All simulation participants must specify the registryUri such that they connect to this registry (e.g., silkit://<IP of node running registry>:8501).
...
The :ref:`participant configuration<sec:ibconfig-json>` file allows to configure changes to the runtime behavior of SIL Kit simulation participants. 
It is a YAML/JSON file through which you can configure a participant to write a trace log file.
...
The file name will be TraceLogFile + a time specific suffix. 
