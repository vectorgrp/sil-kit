...
const std::chrono::milliseconds expectedTimeStep{1*simulationPeriod};
...
private:
    void Setup()
    {
        ...
        if (_participantName.find("P") == 0)
        {
            ...
            participantController->SetSimulationTask([this, publisher](auto now) {
    ...
    void Validate(std::chrono::nanoseconds simTaskNow)
    {
        if (_validation.lastSimTaskTime > std::chrono::nanoseconds::min())
        {
            ...
            //if((_validation.lastSimTaskTime + simulationPeriod) != simTaskNow)
            {
                ...
                throw std::runtime_error{"Validation inside of simTask failed delta=" + delta.count()};
            }
        }
    ...
    void ValidateOnReceive(const std::string& topic,  const std::string& msgStr)
    {
        ...
        if (_validation.lastTime > Nanoseconds::min())
        {
            ...
            if (delta > expectedTimeStep)
            {
                ...
                throw std::runtime_error{"ValidateOnReceive: checking timestamps failed delta=" + delta.count()};
            }
        }
