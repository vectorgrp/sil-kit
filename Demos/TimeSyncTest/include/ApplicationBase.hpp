...
struct Arguments
{
    ...
    bool runAsync{false};
    bool simStepHandlerAsync{false};
...
private:
    ...
    void WorkerThread()
    {
        ...
        while (_lifecycleService->State() == ParticipantState::Running)
        {
            DoWorkAsync();
    ...
    void SetupDoWork()
    {
        if (_arguments.runAsync)
        {
            ...
            _lifecycleService->SetStartingHandler([this]() {
                _unleashWorkerThreadPromise.set_value();
            });
        }
        else
        {
            ...
            if (_arguments.simStepHandlerAsync)
            {
                _timeSyncService->SetSimulationStepHandlerAsync(
    ...
    void Launch() { _participantStateFuture = _lifecycleService->StartLifecycle(); }
...
protected:
    ILifecycleService* GetLifecycleService() const { return _lifecycleService; }

    ITimeSyncService* GetTimeSyncService() const { return _timeSyncService; }

    IParticipant* GetParticipant() const { return _participant.get(); }

    ILogger* GetLogger() const { return _participant->GetLogger(); }

    // Also accessible in app / main
public:
    const Arguments& GetArguments() const { return _arguments; }

    const auto& GetCommandLineParser() { return _commandLineParser; }
    ...
    int Run()
    {
        ...
        {
            std::cerr << "Error: caught a fatal exception: " << error.what() << std::endl;
            return -3;
        }
    }
};
